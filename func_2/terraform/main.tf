...
data "archive_file" "source" {
    ...
    output_path = "play/${var.function_version}/function-source.zip"
}
...
resource "google_cloud_scheduler_job" "play" {
  ...
  http_target {
    body        = base64encode("{\"result\":\"success-play\"}")
    headers = {
      Content-Type = "application/json"
    }
    http_method = "POST"
    uri         = "https://europe-west3-${var.gcp_project_name}.cloudfunctions.net/play"
    oidc_token {
      audience              = "https://europe-west3-${var.gcp_project_name}.cloudfunctions.net/play"
      service_account_email = "project-ga4@nabuminds-test.iam.gserviceaccount.com"
    }
  }
  retry_config {
